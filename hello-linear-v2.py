...
W_hist = tf.compat.v1.summary.histogram("weights", W)
b_hist = tf.compat.v1.summary.histogram("biases", b)
y_hist = tf.compat.v1.summary.histogram("y", y)

y_ = tf.compat.v1.placeholder(tf.float32, [None, 1], name="y_")

# Cost function sum((y_-y)**2)
with tf.compat.v1.name_scope("cost") as scope:
    cost = tf.reduce_mean(input_tensor=tf.square(y_ - y))
    cost_sum = tf.compat.v1.summary.scalar("cost", cost)

# Training using Gradient Descent to minimize cost
with tf.compat.v1.name_scope("train") as scope:
    train_step = tf.compat.v1.train.GradientDescentOptimizer(learn_rate).minimize(cost)
...
writer = tf.compat.v1.summary.FileWriter(log_file, sess.graph_def)
